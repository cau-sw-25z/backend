...
public class PortfolioService {
    ...
    private final PortfolioWeightCalculationService
            portfolioWeightCalculationService;

    @Transactional
    public PortfolioDetailResponse createPortfolio(
        ...
        portfolioItemRepository.saveAllAndFlush(portfolioItems);

        portfolioWeightCalculationService.calculateAndSave(
                portfolio,
                portfolioItems
        );
    ...
    public PortfolioListResponse getMyPortfolios() {
        ...
        List<PortfolioSummaryResponse> items =
                portfolios.stream()
                        .map(portfolio -> {
                            List<PortfolioItem> portfolioItems =
                                    portfolioItemRepository
                                            .findByPortfolio_IdOrderByIdAsc(
                                                    portfolio.getId()
                                            );
    ...
    public PortfolioDetailResponse getPortfolioDetail(
        ...
        Portfolio portfolio =
                getOwnedPortfolio(portfolioId, userId);
    ...
    public void deletePortfolio(Long portfolioId) {
        ...
        portfolioWeightRepository
                .deleteAllByPortfolioId(portfolioId);

        tradeRepository.deleteAllByPortfolioId(portfolioId);

        portfolioItemRepository
                .deleteByPortfolio_Id(portfolioId);
    ...
    public PortfolioDetailResponse addStockToPortfolio(
        ...
        if (portfolioItemRepository
                .existsByPortfolio_IdAndStock_Id(
                        portfolioId,
                        stock.getId()
                )) {
            throw new CustomException(
                    ErrorCode.PORTFOLIO_ITEM_ALREADY_EXISTS
            );
        }

        PortfolioItem portfolioItem =
                portfolioItemRepository.saveAndFlush(
                        new PortfolioItem(
                                portfolio,
                                stock,
                                request.avgPrice(),
    ...
    @Transactional
    public PortfolioDetailResponse deleteStockFromPortfolio(
            Long portfolioId,
            String ticker
    ) {
        Long userId = getCurrentUserId();
        ...
        PortfolioItem portfolioItem =
                portfolioItemRepository
                        .findByPortfolio_IdAndStock_Ticker(
                                portfolioId,
                                normalizedTicker
                        )
                        .orElseThrow(() ->
                                new CustomException(
                                        ErrorCode.PORTFOLIO_ITEM_NOT_FOUND
                                )
                        );

        /*
         * portfolio_weights가 portfolio_items를 FK로 참조하므로
         * 기존 추천 비중을 먼저 삭제합니다.
         */
        portfolioWeightRepository
                .deleteAllByPortfolioId(portfolioId);

        tradeRepository.deleteAllByPortfolioItemId(
                portfolioItem.getId()
        );
        ...
        List<PortfolioItem> remainingItems =
                portfolioItemRepository
                        .findByPortfolio_IdOrderByIdAsc(
                                portfolioId
                        );
    ...
    private PortfolioDetailResponse toDetailResponse(
            Portfolio portfolio,
            List<PortfolioItem> portfolioItems
    ) {
        BigDecimal totalValuation =
                calculateTotalValuation(portfolioItems);

        List<PortfolioDetailResponse.PortfolioItemDetail> items =
                portfolioItems.stream()
                        .map(portfolioItem -> {
                            Stock stock =
                                    portfolioItem.getStock();
    ...
    private BigDecimal calculateTotalValuation(
            List<PortfolioItem> portfolioItems
    ) {
    ...
    private BigDecimal calculateWeightPercent(
            BigDecimal evaluationAmount,
            BigDecimal totalValuation
    ) {
        if (totalValuation.compareTo(
                BigDecimal.ZERO
        ) == 0) {
